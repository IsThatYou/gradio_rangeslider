...
class RangeSlider(FormComponent):
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            labels: Optional list of labels to display for each slider step. Length must match ``maximum - minimum + 1``.
        """
        ...
        self.labels = labels
        if labels and len(labels) != maximum - minimum + 1:
            raise ValueError("labels length must match maximum - minimum + 1")
    ...
    def postprocess(self, value: Tuple[float, float] | None) -> RangeSliderData:
        ...
        return RangeSliderData(
            root=(max(self.minimum, min(value[0], self.maximum)), max(self.minimum, min(value[1], self.maximum)))
        )
